...
export default function CimbarTransfer({ onClose, passwords = [], visible = false }) {
    ...
    useEffect(() => {
        ...
        if (passwordMode === "auto" && !sharePassword) generateRandomPassword();
        else if (passwordMode === "none") setSharePassword("");
    }, [passwordMode, generateRandomPassword]);

    useEffect(() => {
        if (availablePasswords.length > 0 && selectedIds.size === 0) setSelectedIds(new Set(availablePasswords.map((p) => String(p.id || p._id))));
    ...
    const buildPayload = useCallback(async () => {
        ...
        try {
            const result = await tauriAPI.prepareCimbarPayload({ selectedIds: Array.from(selectedIds), sharePassword: sharePassword || null });
    ...
    const restartEngine = async () => {
        ...
        try {
            await loadCimbarEngine(true);
            // 先置 false，再用 setState 回调确保 buildPayload 在新状态下执行
    ...
    const handleDensityChange = (newMode) => {
        if (densityMode === newMode) return;
        setDensityMode(newMode);
        updateCimbarMode(newMode);
        resetEncoderCache(); // clears GLOBAL_LAST_DATA so startEncode re-runs
        setRefreshKey((prev) => prev + 1);
    };
        ...
        <div className="relative h-full w-full bg-white flex flex-col overflow-hidden text-gray-900 font-sans">
            {isZoomed && (
                <div className="absolute inset-0 z-[200] bg-black flex flex-col items-center justify-center p-4 animate-in zoom-in-95 duration-300">
                    <button onClick={() => setIsZoomed(false)} className="absolute top-8 right-8 p-4 bg-white/10 hover:bg-white/20 text-white rounded-2xl z-[210] transition-all">
                        <X className="w-8 h-8" />
                    </button>
            ...
            <div className="flex items-center justify-between px-8 py-4 border-b border-gray-100 bg-white shrink-0">
                ...
                <div className="flex items-center gap-6">
                    <button onClick={restartEngine} className="p-2 hover:bg-blue-50 hover:text-blue-600 text-gray-400 rounded-xl transition-all" title={t("cimbar.restartEngine")}>
                        <RefreshCw className="w-5 h-5" />
                    </button>
                    <div className="bg-gray-100 p-1 rounded-xl flex items-center">
                        <button onClick={() => setMode("generate")} className={cn("px-6 py-1.5 text-xs font-bold rounded-lg transition-all", mode === "generate" ? "bg-white text-blue-600 shadow-sm" : "text-gray-500 hover:text-gray-700")}>
                            {t("actions.generate")}
                        </button>
                        <button onClick={() => setMode("decode")} className={cn("px-6 py-1.5 text-xs font-bold rounded-lg transition-all", mode === "decode" ? "bg-white text-blue-600 shadow-sm" : "text-gray-500 hover:text-gray-700")}>
                            {t("cimbar.decode")}
                        </button>
                    </div>
                ...
                <div className={cn("flex-1 flex transition-opacity duration-300", mode === "generate" ? "opacity-100 visible" : "opacity-0 invisible absolute inset-0")}>
                        ...
                        <div className="p-6 space-y-8 flex-1 overflow-y-auto min-h-0 custom-scrollbar">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-[10px] font-black uppercase tracking-widest">Security</h3>
                                </div>
                                <div className="grid grid-cols-3 gap-1 p-1 bg-gray-50 rounded-lg">
                                    {["none", "fixed", "auto"].map((m) => (
                                        <button key={m} onClick={() => setPasswordMode(m)} className={cn("py-1.5 text-[10px] font-bold rounded-md transition-all", passwordMode === m ? "bg-white text-blue-600 shadow-sm" : "text-gray-400")}>
                                ...
                                {passwordMode !== "none" && (
                                    <div className="relative">
                                        ...
                                        <div className="absolute right-2 top-1.5 flex">
                                            <button onClick={() => setShowPassword(!showPassword)} className="p-1.5 text-gray-400 hover:text-blue-600 transition-colors">
                                                {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                            </button>
                                            {passwordMode === "auto" && (
                                                <button onClick={generateRandomPassword} className="p-1.5 text-blue-500 hover:text-blue-700 transition-colors">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-[10px] font-black uppercase tracking-widest">{t("cimbar.densityLabel")}</h3>
                                </div>
                                <div className="grid grid-cols-4 gap-1 p-1 bg-gray-50 rounded-lg">
                                    {["b", "bm", "bu", "4c"].map((m) => (
                                        <button key={m} onClick={() => handleDensityChange(m)} className={cn("py-1.5 text-[10px] font-bold rounded-md transition-all", densityMode === m ? "bg-white text-blue-600 shadow-sm" : "text-gray-400")}>
                                            {m.toUpperCase()}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-[10px] font-black uppercase tracking-widest">Framerate</h3>
                                    </div>
                                    <span className="text-[10px] font-black text-blue-600 tabular-nums bg-blue-50 px-2 py-0.5 rounded-md">{fps} FPS</span>
                                </div>
                                <input type="range" min="5" max="30" step="1" value={fps} onChange={handleFpsChange} className="w-full h-1.5 bg-gray-100 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                            ...
                            <div className="space-y-4 flex flex-col flex-1 min-h-0">
                                <div className="flex items-center justify-between px-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-[10px] font-black uppercase tracking-widest">Payload</h3>
                                    </div>
                                ...
                                <div className="relative">
                                    <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder={t("cimbar.search")} className="w-full bg-gray-50 border-none rounded-lg pl-10 pr-4 py-2 text-xs outline-none" />
                                        ...
                                        <div key={groupName} className="space-y-0.5">
                                            ...
                                            >
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] font-bold text-gray-400 uppercase">{groupName}</span>
                                                </div>
                                            </div>
                    ...
                    <div className="flex-1 bg-white flex flex-col items-center justify-center relative overflow-hidden">
                        <div className="w-full max-w-[560px] aspect-square flex items-center justify-center bg-white relative group border border-gray-100 shadow-sm rounded-2xl overflow-hidden transition-all duration-500">
                            <CimbarQRCode key={`normal-${refreshKey}`} data={payloadData} filename={payloadFile?.name} className="w-full h-full" visible={visible} densityMode={densityMode} />
                            {payloadData && (
                                <button onClick={() => setIsZoomed(true)} className="absolute -bottom-2 -right-2 p-4 bg-gray-900 text-white rounded-2xl shadow-2xl transition-all opacity-0 group-hover:opacity-100 z-30 flex items-center gap-2">
